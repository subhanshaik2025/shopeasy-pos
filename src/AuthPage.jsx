...
export default function AuthPage({ onLoginSuccess }) {
  ...
  const handleVerify = () => {
    ...
    const result = verifyPhoneNumberFromSheet(phone);
    if (result.success) {
  ...
  const handleRegister = () => {
    if (!phone || !password || !industry) {
      setError('Fill all fields');
      return;
    }
    const result = registerUser(shop, name, phone, password, industry);
    if (result.success) {
    ...
    return (
      <div style={{ background: 'linear-gradient(135deg, #E6F7F5, #F3F3EE)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
        <div style={{ background: '#fff', borderRadius: 16, padding: 32, maxWidth: 400, width: '100%' }}>
          ...
          <button onClick={handleLogin} style={{ width: '100%', padding: 12, borderRadius: 10, fontSize: 14, fontWeight: 700, background: '#0D9488', color: '#fff', border: 'none', cursor: 'pointer', marginBottom: 12 }}>Login</button>
    ...
    return (
      <div style={{ background: 'linear-gradient(135deg, #E6F7F5, #F3F3EE)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
        <div style={{ background: '#fff', borderRadius: 16, padding: 32, maxWidth: 400, width: '100%' }}>
          ...
          <button onClick={handleVerify} style={{ width: '100%', padding: 12, borderRadius: 10, fontSize: 14, fontWeight: 700, background: '#0D9488', color: '#fff', border: 'none', cursor: 'pointer', marginBottom: 12 }}>Verify</button>
      ...
      <div style={{ background: 'linear-gradient(135deg, #E6F7F5, #F3F3EE)', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
        <div style={{ maxWidth: 900, width: '100%' }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            <button onClick={handleRegister} style={{ flex: 1, padding: 12, borderRadius: 10, fontSize: 14, fontWeight: 700, background: '#0D9488', color: '#fff', border: 'none', cursor: 'pointer' }}>Create Account</button>
          </div>
        </div>
      </div>
    );
